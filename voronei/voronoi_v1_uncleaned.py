...
for triangle in triangle_set :
    # if any of the vertices of the triangle are equal to any of the vertices of the super_triangle, remove that triangle
    #if not all([not any([vertex == super_vertex for super_vertex in super_triangle]) for vertex in triangle[0]]) :
    #    triangle_set.remove(triangle)
    if any(out_of_bounds(vert) for vert in triangle[0]) :
        triangle_set.remove(triangle)

# bowyer-watson triangulation done
...
#set = [(0, 0), (0.2, 0.2), (0.5, 0.7), (0.9, 0.1), (0.2, 0.8)]
### now generate voronoi using scipy library
#vor = Voronoi(set)
#def simple_voronoi(vor, lim=None):
#    # Make Voronoi Diagram 
#    fig = voronoi_plot_2d(vor, show_points=True, show_vertices=True, s=num_points)
#
#    # Configure figure 
...
#    plt.show()
#
#simple_voronoi(vor, lim=(0.15, 0.85))
## above code using scipy taken from: https://www.daniweb.com/programming/computer-science/tutorials/520314/how-to-make-quality-voronoi-diagrams
